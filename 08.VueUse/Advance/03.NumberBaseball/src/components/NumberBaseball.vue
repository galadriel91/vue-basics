<template>
    <div>
        ...
        <form @submit.prevent="onSubmitForm">
            <input type="text" placeholder="네자리 숫자를 입력해 주세요" v-model="value" ref="answer" @input="onInputForm" maxlength="4">
...
export default {
    ...
    setup(){
            ...
            if(value.value.length === 4){
                if(value.value === numbers.value.join('')){
                    ...
                    if(tries.value.length >= 9){
                        result.value = `횟수를 초과했습니다. 정답은 ${numbers.value.join('')} 이었습니다.`;
                        value.value = ''
                        tries.value = []
                        numbers.value = getNumber()
                        answer.value.focus()
                        ...
                        let strike = 0;
                        let ball = 0;
                        const valueArray = value.value.split('').map(v=>parseInt(v))
                        for(let i = 0; i < 4 ; i++){
        ...
        return { numbers , value , result , tries , answer , onInputForm , onSubmitForm }
    }
}
</script>
